  ...
  <div>
    ...
    <v-card elevation="0">
      <v-card-title class="justify-content-between">
        Sale Product Category List
        <v-btn
          v-if="isPermit('product-sale-category-add')"
          @click="addDataModel('Add New Sale Product Category')"
          ...
          <div class="table-responsive" v-if="!dataLoading && allData.data">
            <table class="table table-bordered text-center table-striped">
              <thead class="table_head table_head_link">
                <tr>
                  <th>Sale Product Category (English)</th>
                  <th>Sale Product Category (Bangla)</th>
                  <th>Action</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="singleData in allData.data" :key="singleData.id">
                  ...
                  <td class="text-center">
                    <v-btn
                      ...
                      :style="
                        isPermit('product-sale-category-status')
                          ? 'pointer-events: auto;'
                          : 'pointer-events: none'
                      "
                    >
                    ...
                    <v-btn
                      v-if="isPermit('product-sale-category-edit')"
                      @click="
                        editDataModel(
                          singleData,
                          'Update Sale Product Category'
                        )
                      "
                      small
                      class="ma-1 btn_edit"
                    ...
                    <v-btn
                      v-if="isPermit('product-sale-category-delete')"
        ...
        <v-card-text>
          <form @submit.prevent="editmode ? updateData() : createData()">
            <v-text-field
              ...
              label="Sale Product Category Name (English)"
              placeholder="Enter Sale Product Category Name (English)"
              :error-messages="form.errors.get('name_en')"
            ></v-text-field>

            <v-text-field
              v-model="form.name_bn"
              label="Sale Product Category Name (Bangla)"
              placeholder="Enter Sale Product Category Name (Bangla)"
